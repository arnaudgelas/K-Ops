...
def _score_claims() -> dict:
    ...
    try:
        from kops.utils import CONFIG as _cfg
...
def _score_graph_topology() -> dict:
    ...
    isolated_concepts = sum(
        1 for n in con_nodes if in_deg.get(n["id"], 0) + out_deg.get(n["id"], 0) == 0
    )
...
def _score_schema_compliance() -> dict:
    ...
    try:
        ...
        from kops.utils import parse_frontmatter as _pfm
    except ImportError:
...
def compute_scorecard() -> dict:
    ...
    signals = _compute_signals(
        concepts, sources, claims, answers, contradictions, probes, graph_topo
    )
