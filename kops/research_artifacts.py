...
RESEARCH_IMPORTS = CONFIG.research_dir / "imports"
...
def render_research_brief(
    topic: str, slug: str, tier: str, created_at: str, brief_path: Path, status_path: Path
) -> str:
    return "\n".join(
        [
            dump_frontmatter(
...
def render_research_report(
    topic: str, slug: str, tier: str, created_at: str, status_path: Path, review_path: Path
) -> str:
    return dump_frontmatter(
        {
            "title": topic.strip() or slug,
...
def render_import_manifest_bundle(
    ...
    for item in citations:
        body_lines.append(
            f"| {item['url']} | {item['label']} | {item['source_id']} | {item['status']} |"
        )
    if len(citations) == 0:
