...
from kops.ingest_github_repo import github_clone_url, github_home_url, parse_github_repo
from kops.utils import CONFIG, ROOT, find_source_note, load_json, save_json
...
def normalize_source_note(note_path: Path, canonical_repo: str, github_home: str) -> bool:
    ...
    canonical_index = next(
        (i for i, line in enumerate(lines) if line.startswith("- Canonical Repository: `")), None
    )
    home_index = next(
        (i for i, line in enumerate(lines) if line.startswith("- GitHub Home: `")), None
    )
...
def normalize_github_sources(dry_run: bool = False) -> None:
    ...
    for entry in registry:
        ...
        note_path = find_source_note(entry["id"]) or CONFIG.summaries_dir / f"{entry['id']}.md"
...
def main() -> None:
    ...
    normalize_github_sources(dry_run=args.dry_run)
