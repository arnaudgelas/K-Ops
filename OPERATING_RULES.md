...
- An entailment judge (`kops/entailment_judge.py`) exists but is **uncalibrated, advisory, and non-gating**: it produces support verdicts for evaluation only and must not be treated as a deterministic gate or a guarantee of support. Do not wire it into compile/heal/serving gates until `kops/judge_calibration.py` reports a real false-support rate. Only quote existence (`verify-spans`) is deterministic.
...
- When a source's raw `content_hash` changes, propagate it with `python -m kops.invalidation` (optionally `--only <src-id>`, `--dry-run`, `--flag`): it appends a new immutable `SourceVersion`, re-derives claims then contradictions (fixed point), flags dependent claims/answers/concepts stale, appends a `ValidationEvent` per affected object, and writes `data/invalidation_queue.json`. Do not serve stale evidence as current at the `decision` or `autonomous` tier — the serving gate reads that queue to refuse it.
...
- Answers and renders are governed by a declared consequence tier at the output boundary via `ask --tier <tier>` / `render --tier <tier>` (default `exploratory`), not only by the standalone `consequence-gate`. The output gate freezes a content-addressed context package, admits only claims that clear the tier's bar, and maps every factual answer sentence to an admitted claim id; at `decision`/`autonomous` it refuses or qualifies on unsupported, blocked, stale, or uncorroborated evidence and records an audit `ValidationEvent`.
