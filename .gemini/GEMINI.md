...
This repository is an Obsidian-aligned living research vault. The same operating contract should work whether the active CLI is Claude Code, Codex CLI, or Gemini CLI/Antigravity (`agy`).

Canonical rules: see `OPERATING_RULES.md`. This file adds Gemini-specific and Antigravity (`agy`) CLI overrides.

## Gemini-Specific Notes
- Use `uv run kops compile --agent gemini` (or the `/compile` command in `agy`) for wiki compilation passes.
- Use `uv run kops heal --agent gemini` (or the `/heal` command in `agy`) for structural repair and linting.
- Use `uv run kops validate` to confirm config before running workflows.
- Use `uv run kops install-agent-assets --agent gemini` to sync skills and commands to the Gemini/`agy` runtime directory.
...
6. Run `heal` (`uv run kops heal --agent gemini` or the `/heal` command in `agy`) after structural edits.
