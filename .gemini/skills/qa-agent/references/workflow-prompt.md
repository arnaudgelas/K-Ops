...
Answer file: notes/Answers/<timestamp>_<slug>.md

---

## Idempotency guard

Read `notes/Answers/<timestamp>_<slug>.md` first. If the answer section no longer contains `__ANSWER_PENDING__`, the question has already been answered — print the file path and stop. Do not overwrite a completed answer.
...
Replace `__ANSWER_PENDING__` in `notes/Answers/<timestamp>_<slug>.md` with your answer. Keep the rest of the scaffold intact.
...
- [ ] `notes/Answers/<timestamp>_<slug>.md` no longer contains `__ANSWER_PENDING__`.
