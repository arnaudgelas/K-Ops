...
3. **Draft the review**:
   - Write/update `{review_path}`. Preserve its frontmatter (type: `research-review`, `topic_slug`, etc.).
...
Done checklist:
- [ ] Substantive written review saved in `{review_path}` covering objections, missing evidence, and claims to soften.
