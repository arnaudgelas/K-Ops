...
from utils import CONFIG, ROOT, detect_agent_command, ensure_dir, now_stamp, shell_join, short_hash, slugify, write_text
...
RESEARCH_PHASES = {"briefing", "source-collection", "findings", "contrarian-review", "report-drafting", "done", "blocked"}
RESEARCH_TIERS = {"fast", "standard", "deep"}
RESEARCH_STATUS_RE = re.compile(r"^---\n(.*?)\n---\n(.*)$", re.DOTALL)
...
def ensure_research_dirs() -> None:
    for path in [RESEARCH_ROOT, RESEARCH_BRIEFS, RESEARCH_FINDINGS, RESEARCH_NOTES, RESEARCH_REPORTS, RESEARCH_IMPORTS, RESEARCH_ARCHIVE]:
        ensure_dir(path)
...
def render_research_brief(topic: str, slug: str, tier: str, created_at: str, brief_path: Path, status_path: Path) -> str:
    return "\n".join(
        [
            dump_frontmatter(
                {
...
def render_research_findings(topic: str, slug: str, tier: str, created_at: str, status_path: Path) -> str:
    return dump_frontmatter(
        {
            "title": topic.strip() or slug,
...
def render_research_review(topic: str, slug: str, tier: str, created_at: str, status_path: Path) -> str:
    return dump_frontmatter(
        {
            "title": topic.strip() or slug,
...
def render_research_report(topic: str, slug: str, tier: str, created_at: str, status_path: Path, review_path: Path) -> str:
    return dump_frontmatter(
        {
            "title": topic.strip() or slug,
...
def render_research_manifest(topic: str, slug: str, tier: str, archive_date: str, final_phase: str, moved_files: list[str]) -> str:
    return dump_frontmatter(
        {
            "title": topic.strip() or slug,
...
def copy_imported_report(imported_path: Path, topic_slug: str, provider: str, canonical_origin: str) -> tuple[Path, str, str, list[dict[str, str]], Path]:
...
def run_bootstrap(target: str, project_name: str | None = None, with_examples: bool = False, force: bool = False) -> None:
...
def cmd_research_collect(agent: str, topic: str, tier: str = "standard") -> None:
    ...
    prompt = build_runtime_prompt(
        f"research-collect-{slug}",
        "\n".join(
            [
                f"You are collecting sources for the research run `{slug}`.",
                "",
                f"Topic: {topic}",
                f"Quality tier: {tier}",
                f"Brief: {brief_path.relative_to(ROOT)}",
                ...
                f"Progress: {progress_path.relative_to(ROOT)}",
                f"Findings: {findings_path.relative_to(ROOT)}",
                "",
                "Instructions:",
                "- Search broadly first, then narrow to authoritative sources.",
                "- Prefer primary sources over commentary.",
                "- Read full pages for key sources.",
                "- Update existing source notes in `notes/Sources/` instead of creating duplicates.",
                "- Distinguish evidence from inference in the source notes.",
                "- Write or update the findings file with high-signal claims and open questions.",
                "- Treat imported model-generated reports as leads, not authority.",
                "- If you use an imported report, verify its claims against primary sources before promoting anything into a concept page.",
                "- Append a short progress update when you finish.",
                "",
                "Relevant workspace rules live in `CLAUDE.md`.",
            ]
        ),
    )
    agent_run(agent, prompt)
    ...
    set_research_status_phase(slug, topic=topic, tier=tier, phase="findings", brief_path=brief_path, progress_path=progress_path, findings_path=findings_path, report_path=report_path)
...
def cmd_research_review(agent: str, topic: str, tier: str = "standard") -> None:
    ...
    set_research_status_phase(slug, topic=topic, tier=tier, phase="contrarian-review", brief_path=brief_path, progress_path=progress_path, findings_path=findings_path, review_path=review_path, report_path=report_path)
    prompt = build_runtime_prompt(
        f"research-review-{slug}",
        "\n".join(
            [
                f"You are performing a contrarian review for the research run `{slug}`.",
                "",
                f"Topic: {topic}",
                f"Quality tier: {tier}",
                f"Brief: {brief_path.relative_to(ROOT)}",
                ...
                "",
                "Instructions:",
                "- Challenge the current thesis hard.",
                "- Identify contradictions, stale claims, weak evidence, and hidden assumptions.",
                "- Say which claims should be softened or removed.",
                "- Call out missing primary sources.",
                "- Save a substantive written review.",
                "- Append a short progress update when you finish.",
            ]
        ),
    )
...
def cmd_research_report(agent: str, topic: str, tier: str = "standard", require_review: bool = True) -> None:
    slug, brief_path, status_path, progress_path, findings_path, report_path = ensure_research_scaffold(topic, tier)
    review_path = research_review_path(slug)
    if not findings_path.exists():
    ...
    if not report_path.exists():
        write_text(report_path, render_research_report(topic, slug, tier, research_timestamp(), status_path, review_path))
    set_research_status_phase(
        slug,
        topic=topic,
        tier=tier,
    ...
    prompt = build_runtime_prompt(
        ...
        "\n".join(
            [
                f"You are drafting the final report for the research run `{slug}`.",
                "",
                f"Topic: {topic}",
                f"Quality tier: {tier}",
                f"Brief: {brief_path.relative_to(ROOT)}",
                ...
                "",
                "Instructions:",
                "- Read the brief, findings, source notes, and contrarian review first.",
                "- Build the report from collected evidence, not memory.",
                "- Keep conclusions bounded by the available evidence.",
                "- Treat imported model-generated reports as leads, not authority.",
                "- Verify any imported-report claim against primary sources before relying on it.",
                "- Save the final report at the path above.",
                "- Append a short progress update when you finish.",
            ]
        ),
    )
    ...
    set_research_status_phase(slug, topic=topic, tier=tier, phase="done", brief_path=brief_path, progress_path=progress_path, findings_path=findings_path, review_path=review_path, report_path=report_path)
...
def cmd_research_import(
    ...
    set_research_status_phase(slug, topic=topic, tier=tier, phase="source-collection", brief_path=brief_path, progress_path=progress_path, findings_path=findings_path, report_path=report_path)
...
def cmd_research_archive(topic: str) -> None:
    ...
    if archive_root.exists() and any(archive_root.iterdir()):
        raise RuntimeError(f"Archive destination already exists and is not empty: {archive_root.relative_to(ROOT)}")
    ...
    imports_dir = ROOT / str(frontmatter["imports_path"]) if frontmatter.get("imports_path") else research_import_dir(slug)
...
def main() -> None:
    ...
    p_research_import.add_argument("--provider", choices=["gemini", "openai", "claude", "perplexity", "other"], default="other")
    ...
    p_bootstrap.add_argument("--force", "--froce", action="store_true", help="Overwrite the starter scaffold even if the target already exists.")
    ...
    if args.command == "ingest":
        ...
        cmd_research_report(args.agent, args.topic, tier=args.tier, require_review=not args.allow_missing_review)
    elif args.command == "research-import":
        cmd_research_import(args.topic, args.path, args.provider, canonical_origin=args.origin, tier=args.tier)
        ...
        run_bootstrap(target=args.target, project_name=args.project_name, with_examples=args.with_examples, force=args.force)
